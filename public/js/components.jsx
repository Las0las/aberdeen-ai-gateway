...
function RightPanel({state,dispatch,onClose}){
  ...
  const {panelTab,policies,events,dlq,activeAgent,dashboardStats}=state;
  const allAgents=[...AGENTS,...state.customAgents];
  return(<div className="panel" ref={panelRef}>
    <div className="panel-handle"/>
    <div className="panel-tabs">{['config','prompt','policies','events','dlq','metrics','dashboard'].map(tab=>(<button key={tab} className={`panel-tab${panelTab===tab?' active':''}`} onClick={()=>dispatch({type:'SET_PANEL_TAB',payload:tab})}>{tab==='dlq'?'DLQ':tab==='dashboard'?'📊':tab.charAt(0).toUpperCase()+tab.slice(1)}</button>))}</div>
      ...
      {panelTab==='dashboard'&&<div className="dashboard-panel">
        ...
        {dashboardStats&&<>
          ...
          <div className="metric-grid">
            <div className="metric-card"><div className="metric-value" style={{color:'var(--vice-blue,var(--accent))'}}>{dashboardStats.candidates?.toLocaleString()??'—'}</div><div className="metric-label">Candidates</div></div>
            <div className="metric-card"><div className="metric-value" style={{color:'var(--danger)'}}>{dashboardStats.jobs?.toLocaleString()??'—'}</div><div className="metric-label">Active Jobs</div></div>
            <div className="metric-card"><div className="metric-value" style={{color:'var(--accent)'}}>{dashboardStats.placements?.toLocaleString()??'—'}</div><div className="metric-label">Placements</div></div>
            <div className="metric-card"><div className="metric-value" style={{color:'var(--danger)'}}>{dashboardStats.gatewayRequests?.toLocaleString()??'—'}</div><div className="metric-label">AI Requests</div></div>
          </div>
          <h4 style={{fontSize:12,fontWeight:600,marginTop:16,marginBottom:8,color:'var(--text-secondary)',textTransform:'uppercase',letterSpacing:'0.5px'}}>Recent AI Gateway Requests</h4>
          {dashboardStats.recentRequests&&dashboardStats.recentRequests.length>0?
            dashboardStats.recentRequests.map((r,i)=>(<div key={i} className="event-item"><div className="event-dot" style={{background:r.response_status==='success'?'var(--accent)':'var(--danger)'}}/><div><div className="event-text">{r.intent||'chat'} — {r.model_used||'claude'}</div><div className="event-time">{r.latency_ms?r.latency_ms+'ms':''} · {r.response_status} · {new Date(r.created_at).toLocaleTimeString()}</div></div></div>))
            :<div style={{fontSize:12,color:'var(--text-muted)',padding:12,textAlign:'center'}}>No AI requests logged yet. Start chatting to generate data.</div>}
...
function App(){
  ...
  useEffect(()=>{if(state.panelTab==='dashboard'&&!state.dashboardStats){fetchDashboardStats().then(stats=>{dispatch({type:'SET_DASHBOARD_STATS',payload:stats})}).catch(()=>{})}},[state.panelTab,state.dashboardStats]);
