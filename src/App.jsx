...
function EntityCreateModal({ config, onClose, onSave }) {
  ...
  const save = async () => {
    ...
    try { await fetch(`https://lalkdgljfkgiojfbreyq.supabase.co/rest/v1/${config.table}`, { method: "POST", headers: { apikey: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxhbGtkZ2xqZmtnaW9qZmJyZXlxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE1NTIwNTEsImV4cCI6MjA4NzEyODA1MX0.6Cf1fnYL2jaJORCfaf_uGrJ3f2Xkf_k6pC8j34fWfXY", "Content-Type": "application/json", Prefer: "return=minimal" }, body: JSON.stringify({ title: rec.title || rec.name, status: rec.status, location: rec.location }) }); } catch (e) {}
